    ...
    while (fgets(user_input, USER_INPUT_MAX_SIZE, stdin)) {
        ...
        if (strncmp(user_input, "/login", 6) == 0) {
            ...
            if (response.type == JN_ACK) {
                ...
                // start_polling(); // this is old, before adding pms
            ...
            exit(0);
        } else if (strncmp(user_input, "/privmsg", 8) == 0) {
            if (!logged_in) {
                printf("You are not logged in yet.\n");
                goto loop_end;
            }

            char dest_user[MAX_NAME];
            char msg[MAX_DATA];
            if (sscanf(user_input, "/privmsg %s %[^\n]", dest_user, msg) < 2) {
                printf("Please make sure format is: /privmsg <dest-user> <message>\n");
                goto loop_end;
            }

            if (strcmp(client_id, dest_user) == 0) {
                printf("You cannot send a private message to yourself.\n");
                goto loop_end;
            }
